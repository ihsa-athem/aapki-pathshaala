...
const DEFAULT_VIDEO_HEIGHT = 210   // px — leaves plenty of room for the transcript
...
export default function VideoPanel({ youtubeId, chunks, isLoading, language, onSeek, playerRef, chapters, loadingStatus, funFacts = [] }) {
  ...
  return (
    <div ref={panelRef} className="flex flex-col h-full bg-white min-w-0" style={{ borderRight: '1px solid #e5e7eb' }}>
      ...
      <div
        className="flex-shrink-0 h-4 bg-gray-100 hover:bg-indigo-50 cursor-row-resize
                   flex items-center justify-center gap-1 border-y border-gray-200 group select-none
                   transition-colors duration-150"
        onMouseDown={startDrag}
        onTouchStart={startDrag}
        title="Drag to resize video / transcript"
      >
        <div className="flex gap-0.5">
          {[0,1,2,3,4,5].map(i => (
            <div key={i} className="w-3 h-0.5 rounded-full bg-gray-300 group-hover:bg-indigo-400 transition-colors" />
          ))}
        </div>
        <span className="text-[9px] font-medium text-gray-400 group-hover:text-indigo-400 transition-colors uppercase tracking-widest select-none">
          drag
        </span>
      </div>
      ...
      <div className="flex-1 flex flex-col min-h-0 relative">
        <div className="px-3 py-2 border-b border-gray-200 bg-white flex items-center gap-2 flex-shrink-0 min-w-0 shadow-sm">
          <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest flex-shrink-0">
              ...
              {visibleChunks.map((chunk, idx) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      'w-full text-left flex gap-3 px-4 py-3 border-l-[3px] transition-all duration-150',
                      animateChunks ? 'transcript-line-in' : '',
                      isActive
                        ? 'bg-amber-50 border-amber-400'
                        : 'border-transparent hover:bg-slate-50 hover:border-slate-200',
                    ].join(' ')}
                    style={animateChunks ? { animationDelay: `${idx * 40}ms` } : {}}
                  >
                    {/* Timestamp pill */}
                    <span className={[
                      'flex-shrink-0 text-[11px] font-mono tabular-nums mt-0.5 w-9',
                      'px-1 py-0.5 rounded text-center leading-none self-start',
